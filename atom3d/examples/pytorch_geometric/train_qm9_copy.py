...
sys.path.append('../../atom3d/datasets/smp')
...
class Net(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        #print(out.shape)  # (68, 64)
        #print(data.batch)
        #input()

        out = self.set2set(out, data.batch)

        print(out.shape)  # (4, 128)
        input()
...
def main(target = 0, dim = 64, prefix='mu', seed=42):
    ...
    path = osp.join(osp.dirname(osp.realpath(__file__)), '/oak/stanford/groups/rondror/projects/atom3d/supporting_files/small_molecule_properties')
    ...
    logging.info('Loading split from '+ path + '/splits/')

...
from models.heads import EqvLEPFeedForward
from sklearn.metrics import roc_auc_score, average_precision_score
from utils import get_mask, get_ff_mask
import data.lep.eqv_lep_dataloader as dl
import os
import time
import numpy as np
import torch
...
def train(epoch, transformer_model, ff_model, loader, criterion, optimizer, device, max_train_iter, print_frequency):
    ...
    for it, (active, inactive) in enumerate(loader):
        ...
        active.pos, inactive.pos = active.pos.to(device), inactive.pos.to(device)
        active.elements, inactive.elements = active.elements.to(device), inactive.elements.to(device)
        optimizer.zero_grad()

        mask1 = get_mask(active.pos)
        mask2 = get_mask(inactive.pos)
        out_active = transformer_model(active.pos, active.elements, attn_mask=mask1)
        out_inactive = transformer_model(inactive.pos, inactive.elements, attn_mask=mask2)
        ff_mask = get_ff_mask(active.pos, inactive.pos)
        output = ff_model(out_active, out_inactive, active.pos, inactive.pos, ff_mask)

        loss = criterion(output.float(), active.label.float())
        loss.backward()
...
def test(transformer_model, ff_model, loader, criterion, device, max_test_iter, print_frequency):
    ...
    for it, (active, inactive) in enumerate(loader):
        ...
        weights = np.ones(output.shape)
        labels = active.label.float().cpu()
...
def train_eqv_lep(ex, use_attention, data_dir, device, log_dir, checkpoint, num_epochs, batch_size, num_dense,
    ...
    ff_model = EqvLEPFeedForward(Rs_hidden, Rs_ff, Rs_out, max_radius, number_of_basis,
                            h, L).to(device)

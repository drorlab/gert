...
from models.heads import RESFeedForward
from utils import get_mask, get_acc, get_top_k_acc
from sklearn.metrics import f1_score
import data.res.res_dataloader as dl
import os
import time
import numpy as np
import torch
...
def train(epoch, transformer_model, ff_model, loader, criterion, optimizer, device, max_train_iter, print_frequency):
    ...
    for it, subunit in enumerate(loader):
        ...
        subunit.class_weights = subunit.class_weights.to(device)
        optimizer.zero_grad()
        mask = get_mask(subunit.pos)
        out = transformer_model(subunit.pos, mask)
        out = ff_model(out)
        class_weights = torch.mean(subunit.class_weights, dim=0)
...
def test(transformer_model, ff_model, loader, criterion, device, max_test_iter, print_frequency):
    ...
    for i, subunit in enumerate(loader):
        ...
        out = transformer_model(subunit.pos, mask)
        out = ff_model(out)
        test_loss = criterion(out, subunit.label)
...
def train_noneqv_res(ex, use_attention, data_dir, device, log_dir, checkpoint, num_epochs, batch_size,
    ...
    ff_model = RESFeedForward(d_model=hidden_dim, n_out=len(RES_LABEL)).to(device)
    ...
    if checkpoint:
        ...
        transformer_model.load_state_dict(cpt['model_state_dict'])

    ...
    def __iter__(self):
        ...
        if worker_info is None:  # single-process data loading, return the full iterator
            gen = dataset_generator(self.sharded, range(self.num_shards), 
                                    self.labels, self.max_radius, shuffle=True)
...
def df_to_graph(target_df, label, max_radius):
    ...
    one_hot = torch.zeros(elements.shape[0], len(PROT_ATOMS) + len(RES_LABEL))
    ...
    return AtomEnvironment(torch.from_numpy(atom_pos), one_hot.float(), label)
...
def custom_collate(data_list):
    ...
    label = torch.tensor([atom_env.label for atom_env in data_list])

    return AtomEnvironment(pos, elements, label)

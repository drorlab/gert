...
class AtomEnvironment:
    def __init__(self, neighbors, label, mask=None):
...
def custom_collate(data_list):
    ...
    pos_1 = pad_sequence([atom_env.neighbors for atom_env in batch_1]).permute(1, 0, 2)
    labels_1 = torch.tensor([atom_env.label for atom_env in batch_1])

    pos_2 = pad_sequence([atom_env.neighbors for atom_env in batch_2]).permute(1, 0, 2)

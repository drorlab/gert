...
def dataset_generator(sharded, shard_indices, max_radius, shuffle=True):
    ...
    for shard_idx in shard_indices:
        ...
        for i, (ensemble_name, target_df) in enumerate(shard.groupby(['ensemble'])):
            sub_names, (_, _, unbound1, unbound2) = nb.get_subunits(target_df)
            ...
            for _, row in labels.iterrows():
                ...
                graph1 = df_to_graph(unbound1, chain_res1, label, max_radius)
                if graph1 is None:
                    continue
                graph2 = df_to_graph(unbound2, chain_res2, label, max_radius)
                if graph2 is None:
                    continue
                yield graph1, graph2
